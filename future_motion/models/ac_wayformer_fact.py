...
class WayformerFact(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        decoder["hidden_dim"] = decoder_hidden_dim
...
class InputProjections(nn.Module):
    ...
    def forward(
        ...
        Returns:
            target_emb: [n_batch, n_step_hist, hidden_dim]
            target_valid: [n_batch, n_step_hist]
            other_emb: [n_batch, n_other, n_step_hist, hidden_dim]
            other_valid: [n_batch, n_other, n_step_hist]
            tl_emb: [n_batch, n_tl, n_step_hist, hidden_dim]
            tl_valid: [n_batch, n_tl, n_step_hist]
            map_emb: [n_batch, n_map, n_pl_node, hidden_dim]
...
class FactorizedEarlyFusionEncoder(nn.Module):
    def __init__(
        ...
        n_temp_latent_query: int,
        n_spat_latent_query: int,
    ...
    def forward(
        ...
        Args:
            ...
            other_emb: [n_batch, n_other, n_step_hist, hidden_dim]
            other_valid: [n_batch, n_other, n_step_hist]
            tl_emb: [n_batch, n_step_hist, n_tl, hidden_dim]
            tl_valid: [n_batch, n_step_hist, n_tl]
            map_emb: [n_batch, n_map, n_pl_node, hidden_dim]
            map_valid: [n_batch, n_map, n_pl_node]
        ...
        n_batch = other_emb.shape[0]
        n_other = other_emb.shape[1]
        n_tl = tl_emb.shape[2]

        temp_emb = torch.cat([target_emb, other_emb.flatten(0, 1), tl_emb.permute(0,2,1,3).flatten(0, 1)], dim=0)
        temp_emb_invalid = ~torch.cat([target_valid, other_valid.flatten(0, 1), tl_valid.permute(0,2,1).flatten(0, 1)], dim=0)

        # Gather validity for per-entity temporal latent queries
        target_any = target_valid.any(1)
        other_any  = other_valid.any(2).flatten(0, 1)
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.tf_anchor = TransformerBlock(
            d_model=hidden_dim,
            d_feedforward=hidden_dim * n_ff_mult,
